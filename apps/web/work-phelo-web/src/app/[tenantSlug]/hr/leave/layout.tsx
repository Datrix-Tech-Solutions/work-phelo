...
import { useLeaveSidebarGroups } from '@/hooks/hr/useLeaveSidebarGroups';
import { useHrSidebarGroups } from '@/hooks/hr/useHrSidebarGroups';
import { AppBackground } from '@/components/atoms/AppBackground';
import { useModuleThemeScope } from '@/hooks';

/**
 * Leave's own dedicated layout/sidebar — same navigation pattern as
 * Accounting/Operations/Payroll/Appraisal (a full multi-page sidebar instead
 * of one page with tabs), even though Leave is still an HR section under
 * /hr/leave. HR's own layout hands off its chrome to this one for anything
 * under here — see the pathname check in hr/layout.tsx.
 */
export default function LeaveLayout({
  ...
  const { groups } = useLeaveSidebarGroups(tenantSlug);

  // Parked, icon-only HR rail — same as the one shown from Accounting/Operations/Payroll/Appraisal.
