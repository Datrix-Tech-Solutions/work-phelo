...
export default function NewInvoicePage({ params }: { params: Promise<{ tenantSlug: string }> }) {
    ...
    <div className="flex flex-col h-full overflow-hidden">
      <div className={`${pageBreadcrumb} shrink-0 flex items-center`}>
        <nav className="flex items-center gap-2 text-sm text-gray-400">
          <Link
            href={`/${tenantSlug}/accounting/accountspayable`}
      ...
      <div className={`${pageContent} flex-1 min-h-0 overflow-y-auto`}>
        <NewInvoiceForm
          side="PAYABLE"
          onCancel={() => router.push(`/${tenantSlug}/accounting/accountspayable`)}
          onCreated={() => router.push(`/${tenantSlug}/accounting/accountspayable`)}
        />
      </div>
    </div>
  );
