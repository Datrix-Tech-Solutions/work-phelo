...
import { BalanceSheetView } from '@/components/organisms/accounting/BalanceSheetView';

export default function BalanceSheetPage({ params }: { params: Promise<{ tenantSlug: string }> }) {
  ...
  return (
    <div className="flex flex-col gap-6 py-6 pr-6 pl-(--page-pl) min-h-0 overflow-y-auto flex-1">
      <nav className="flex items-center gap-2 text-sm text-gray-400 shrink-0">
        ...
        <span className="text-gray-700 font-medium">Balance Sheet</span>
      </nav>
      <BalanceSheetView />
    </div>
  );
