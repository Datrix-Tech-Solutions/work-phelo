...
interface ReinsurersReportTableProps {
  rows: ReinsurerReportRow[];
  currencySymbol: ReinsurersReportSummary['currencySymbol'];
  isLoading: boolean;
}
...
export function ReinsurersReportTable({
  ...
  const columns: Column<ReinsurerReportRow & { id: string }>[] = useMemo(
    () => [
      ...
      },
      {
        key: 'cededPremium',
        label: 'Ceded Premium',
        render: (row) => fmtAmount(row.cededPremium, currencySymbol),
      },
      {
        key: 'outstanding',
        label: 'Outstanding',
  ...
  return (
    <DataTable
      ...
      emptyMessage="No reinsurer activity for the selected filters"
      currentPage={page}
