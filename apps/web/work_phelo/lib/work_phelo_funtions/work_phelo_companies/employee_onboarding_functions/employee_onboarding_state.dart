...
class UserNotifier extends StateNotifier<EmployeeState> {
  ...
  void updateUser(EmployeeModel updated) {
  ...
  int daysUntilReset(EmployeeModel user) {
    final now = DateTime.now();
...
final activeUsersProvider = Provider<List<EmployeeModel>>((ref) {
  return ref
      .watch(userProvider)
      .users
...
final onLeaveUsersProvider = Provider<List<EmployeeModel>>((ref) {
  return ref
      .watch(userProvider)
      .users
...
final usersByTenantProvider = Provider.family<List<EmployeeModel>, String>((
  ref,
  tenantSlug,
) {
  return ref
